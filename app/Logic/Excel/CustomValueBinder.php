...
class CustomValueBinder extends \PHPExcel_Cell_DefaultValueBinder implements \PHPExcel_Cell_IValueBinder
{
    ...
    public function bindValue(\PHPExcel_Cell $cell, $value = null)
    {
        ...
        // Check for number with comma  这里是因为导出财务报表的时候，需要数字表现为会计形式，所以强制优先判断这个
        ...
        if ($dataType === \PHPExcel_Cell_DataType::TYPE_STRING && !$value instanceof \PHPExcel_RichText) {
            ...
            if (preg_match('/^\-?[0-9]*\.?[0-9]*\s?\%$/', $value)) {
                ...
                $cell->setValueExplicit( $value, \PHPExcel_Cell_DataType::TYPE_NUMERIC);
                // Set style
                $t = $cell->getWorksheet()->getStyle( $cell->getCoordinate() )
                    ->getNumberFormat();
                if ($value == 0) {
                    $t->setFormatCode( \PHPExcel_Style_NumberFormat::FORMAT_PERCENTAGE );
                }else {
                    $t->setFormatCode( \PHPExcel_Style_NumberFormat::FORMAT_PERCENTAGE_00 );
                }
                return true;
            }
